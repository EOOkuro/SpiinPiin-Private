...
public class CountryTest {
	...
	@Test
	public void verifyKEN() {
		try {
			WebTarget target = this.client.target(this.baseUrl + "lcountry");
			Response respose = target.request().get();
			if (respose.getStatus() != 200){
				throw new Exception("Login failed! " + respose.getStatus());
			}
			...
			for (int i = 0; i < obj.size(); i++) {
				if (((JSONObject)obj.get(i)).get("code").toString().equalsIgnoreCase("KEN")){
					found = true;
				}
			}
			
			assertTrue("Country Kenya found!", found);
		} catch (Exception e) {
